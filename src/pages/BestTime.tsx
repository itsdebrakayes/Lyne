  ...
  const { data: congestionData, isLoading: loadingData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: insights } = await supabase
        ...
        .eq('insight_type', 'peak_hours')
        .order('generated_at', { ascending: false })
        .limit(1)
        .single();

      if (insights?.data) {
        const insightData = insights.data as unknown as { heatmap: CongestionData[] };
        return insightData;
      }
      ...
      for (const visit of visits || []) {
        const key = `${visit.day_of_week}-${visit.hour_of_day}`;
        const existing = aggregated.get(key) || { total: 0, count: 0 };
        existing.total += visit.wait_time_minutes || 0;
        existing.count++;
      ...
      for (const day of [0, 1, 2, 3, 4, 5, 6]) {
        for (const hour of HOURS) {
          ...
          const avgWait = stats ? Math.round(stats.total / stats.count) : 0;
