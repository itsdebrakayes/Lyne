...
const JoinQueue = () => {
  ...
  const joinQueue = async (userId: string, clientId: string) => {
    ...
    const estimatedWait = position * (service.base_avg_time_minutes || 5);
    ...
    if (lineError) {
      console.error('Error joining queue:', lineError);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const lineData = await joinQueue(user.id, client.id);
      if (lineData) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // Create client record with Jamaican form data
