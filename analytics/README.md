# Analytics Notebooks Integration

This directory contains the Jupyter notebook infrastructure for advanced analytics processing.
...
```bash
curl -X POST "https://xghhumucsccbfmpdexsr.supabase.co/functions/v1/export-table-data" \
...
```bash
curl -X POST "https://xghhumucsccbfmpdexsr.supabase.co/functions/v1/import-insights" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_SERVICE_ROLE_KEY" \
  -d '{
    "organization_id": "YOUR_ORG_ID",
