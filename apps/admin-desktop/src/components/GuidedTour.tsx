...
export default function GuidedTour({ role }: { role: Role }) {
  ...
  const tooltipTop = rect ? Math.min(window.innerHeight - 190, Math.max(24, rect.bottom + 18)) : 120;
  const tooltipLeft = rect ? Math.min(window.innerWidth - 380, Math.max(24, rect.left)) : 40;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, pointerEvents: 'none' }} aria-live="polite">
      ...
      <div role="dialog" aria-label="Dashboard tutorial" style={{ position: 'fixed', top: tooltipTop, left: tooltipLeft, width: 340, maxWidth: 'calc(100vw - 48px)', background: '#fff', color: '#121216', borderRadius: 8, padding: 20, boxShadow: '0 18px 60px rgba(0,0,0,.28)', pointerEvents: 'auto' }}>
