...
export function StaffAssignmentEditor({ staff, organizationId }: StaffAssignmentEditorProps) {
  ...
  // Fetch services for this organization via MySQL backend
  const { data: services = [] } = useQuery({
    queryKey: ['services', organizationId],
    queryFn: async () => api.get<Service[]>(`/services?business_id=${organizationId}`),
    enabled: isOpen,
  });

  // Fetch counters for this organization via MySQL backend
  const { data: counters = [] } = useQuery({
    queryKey: ['counters', organizationId],
    queryFn: async () => api.get<Counter[]>(`/branches/counters?business_id=${organizationId}`),
    enabled: isOpen,
  });
  ...
  // Mutation to update staff assignment via MySQL backend
